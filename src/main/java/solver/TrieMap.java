...
public class TrieMap implements Map<Long, SimpleTrie> {
     ...
     */
    public static SimpleTrie query(SimpleTrie source, boolean[] isConstant, long[] constantArr, int[][] sameArr) {
        return cloneSatisfyingParts(source, 0, isConstant, constantArr, sameArr);
    }
     ...
     * @return
     */
    private static SimpleTrie cloneSatisfyingParts(SimpleTrie from, int index, boolean[] isConstant, long[] constantArr, int[][] sameArr) {
        if (from == null) {
            return null;
        }
        ...
        if (isConstant[index]) {
            return cloneSatisfyingParts(from.children.get(constantArr[index]), index + 1, isConstant, constantArr, sameArr);
        }
...
//            If we have a requirement that the same variable must occur later on bind it.
            ...
            var child = cloneSatisfyingParts(from.children.get(x), index + 1, isConstant, constantArr, sameArr);
...
//        If there were no variables so far we just return the matches for the next atom
        if (old.children == null && old.leaves == null) {
            return query(atomSolution, atom.constBool, atom.constArr, atom.sameNess);
        }
